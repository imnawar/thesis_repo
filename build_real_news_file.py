...
def get_actual_images(images_folder):
    """Return the real list of image filenames present on disk, correctly ordered
    (main_image.* first, then 1.jpg/2.jpg/3.jpg... in numeric order)."""
...
def build_dataset(root_folder):
    ...
    for folder_name in os.listdir(root_folder):
        ...
        # Parse file (images_from_txt is the declared list, e.g. 1.jpg/2.jpg — kept only
        # to know how many images were referenced; NOT used for the final path)
        title, images_from_txt, captions, image_urls, date, full_text = parse_context_file(context_path)

        # 🔴 Debug (important)
        if not title:
            print(f"Warning: No title in {folder_name}")

        # 🔧 MAPPING FIX: get the real filenames from disk instead of trusting
        # the names written inside content.txt, since real folders don't always
        # follow the 1.jpg/2.jpg/3.jpg convention (some use main_image.jpg + 2.jpg + 3.jpg).
        ...
        # Match items safely — position i in actual_images maps to position i
        # in captions/image_urls (both come from content.txt, in original order)
        num_items = min(len(actual_images), len(captions), len(image_urls))
        if num_items == 0:
            print(f"Skipping (no valid pairs): {folder_name}")
            continue

        if len(actual_images) != len(images_from_txt):
            print(f"Note: declared vs actual image count differs in {folder_name} "
                  f"(declared={len(images_from_txt)}, actual={len(actual_images)})")

...
def load_data(csv_path=CSV_PATH):
    ...
    # --- Label convention swap ---------------------------------------------
    # Original CSV convention (and the rest of this codebase, before this
    # edit) treated fake news = 1, real news = 0. We flip it ONCE here, at
    # the source, so that everywhere downstream: real news = 1, fake news = 0.
...
def split_data_no_leakage(df, test_size=0.2, val_size=0.1, seed=42):
    ...
    for name, split in [("train", train), ("val", val), ("test", test)]:
        ...
        print(f"{name}: {len(split)} rows, real-news rate = {rate:.3f}")
...
def evaluate_zero_shot(df_with_sims, threshold=None):
    """
    Full zero-shot metric suite: AUC, Accuracy, Precision, Recall.
    Positive class = real news (label == 1). Higher CLIP similarity now
    scores toward "real" directly (no negation), since real=1 is the
    positive class we're ranking for.

    Zero-shot has no learned decision boundary, so there's no threshold to
    apply out of the box. If `threshold` isn't given, we pick the one that
    maximizes Youden's J (TPR - FPR) on this same data via the ROC curve.
    CAVEAT: choosing the threshold on the same set you evaluate on is
    optimistic — it reports the *best achievable* operating point, not a
    threshold that would generalize. Report this alongside AUC (which is
    threshold-free) rather than as a stand-in for held-out performance.
    """
    from sklearn.metrics import roc_curve

    valid = df_with_sims.dropna(subset=["clip_similarity"])
    labels = valid["label"].values
    scores = valid["clip_similarity"].values
    ...
    if threshold is None:
        ...
        youden_j = tpr - fpr
        threshold = thresholds[np.argmax(youden_j)]
    ...
    print("Zero-shot M-CLIP similarity — results (positive class = real news):")
...
class NewsPairDataset(Dataset):
    def __init__(self, df, image_preprocess, tier_probs=(0.5, 0.3, 0.2)):
        self.real_df = df[df.label == 1].reset_index(drop=True)
        self.fake_df = df[df.label == 0].reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    zero_shot_metrics = evaluate_zero_shot(df_scored)
    ...
    classifier_metrics = evaluate_classifier(classifier, test_cache)

    print("\n=== Zero-shot vs. classifier (positive class = real news) ===")
    print(f"{'metric':<10} {'zero-shot':>10} {'classifier':>12}")
    for k in ("accuracy", "precision", "recall", "auc"):
        print(f"{k:<10} {zero_shot_metrics[k]:>10.4f} {classifier_metrics[k]:>12.4f}")
